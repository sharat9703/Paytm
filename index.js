     ...
     checksum_lib.genchecksum(params, config.PaytmConfig.key, function (err, checksum) {
       ...
       let post_req = https.request(options, function(post_res) {
         ...
         post_res.on('end', function(){
           ...
           let _results = JSON.parse(response);
           /* where it will come back after payment*/
           res.redirect(`https://amazon--clone-site.herokuapp.com/viewBooking?status=${_results.STATUS}&ORDERID=${_results.ORDERID}&date=${_results.TXNDATE}&bank=${_results.BANKNAME}`)
           });
       });
